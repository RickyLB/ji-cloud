...
impl SidebarSpot {
    ...
    pub fn new_pro_dev_unit(unit: Option<ProDevUnit>) -> Rc<Self> {
        Rc::new(Self {
            is_incomplete: Mutable::new(match &unit {
                Some(unit) => !unit.is_complete,
                None => false,
            }),
            item: SidebarSpotItem::ProDev(unit.map(|unit| Rc::new(unit))),
        })
    }
