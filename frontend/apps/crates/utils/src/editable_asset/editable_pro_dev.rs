...
use shared::domain::{
    category::CategoryId,
    pro_dev::{unit::ProDevUnitId, ProDevId, ProDevResponse, ProDevUpdateDraftDataRequest},
    module::LiteModule,
};
...
pub struct EditableProDev {
    ...
    pub units: MutableVec<ProDevUnitId>,
...
impl EditableProDev {
    pub fn fill_from_pro_dev(&self, pro_dev: ProDevResponse) {
        ...
        self.units.lock_mut().replace(pro_dev.pro_dev_data.units);
        self.display_name.set(pro_dev.pro_dev_data.display_name);
        self.description.set(pro_dev.pro_dev_data.description.clone());
